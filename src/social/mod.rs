...
struct SocialProfileResponse {
    ...
    avatar_optimized_urls: Option<OptimizedMediaUrls>,
}
...
#[derive(Debug, FromRow)]
struct SocialMediaAttachmentRow {
    id: Uuid,
    original_file_name: String,
    mime_type: String,
    size_bytes: i64,
...
async fn scope() -> Json<ScopeResponse> {
    Json(ScopeResponse {
        ...
        implemented_now: vec![
            ...
            "optimized-social-media-urls",
            "optimized-social-avatar-urls",
...
async fn create_post(
    ...
    if body.is_empty() && media_asset_ids.is_empty() {
        return Err(ApiError::BadRequest(
            "post body or media attachment is required".to_string(),
        ));
    }
    ...
    if outcome.is_block() {
        moderation::record_content_decision(
            &state,
            Some(user.id),
            "post",
            Some(post_id),
    ...
    let status = if outcome.is_review() {
        "hidden"
    } else {
        "published"
    };
...
async fn create_comment(
    ...
    attach_media_assets(
        ...
        "social_comment",
        comment_id,
        payload.media_asset_ids.unwrap_or_default(),
    )
    .await?;
...
fn social_profile_response(
    ...
    let avatar_optimized_urls = avatar_url
        .as_deref()
        .and_then(|url| optimized_public_image_urls(config, url, "image/webp"));
...
async fn hydrate_post(
    ...
    Ok(HydratedPostResponse {
        ...
        author: social_profile_response(&state.config,
            row.author_user_id,
...
) -> Result<HydratedCommentResponse, ApiError> {
...
async fn fetch_media_for_entity(state: &AppState, entity_type: &str, entity_id: Uuid, viewer_id: Option<Uuid>) -> Result<Vec<SocialMediaAttachment>, ApiError> {
    let rows = sqlx::query_as::<_, SocialMediaAttachmentRow>(
        r#"
    ...
    Ok(rows
        .into_iter()
        .map(|row| {
            let optimized_urls = row
                .public_url
                .as_deref()
                .and_then(|url| optimized_public_image_urls(&state.config, url, &row.mime_type));
...
async fn attach_media_assets(
    state: &AppState,
    owner_user_id: Uuid,
    entity_type: &str,
    entity_id: Uuid,
    media_asset_ids: Vec<Uuid>,
) -> Result<(), ApiError> {
...
async fn visible_profile(state: &AppState, user_id: Uuid) -> Result<SocialProfileResponse, ApiError> {
    let row = sqlx::query_as::<_, SocialProfileRow>(
        r#"
...
async fn ensure_visible_post_exists(state: &AppState, post_id: Uuid) -> Result<(), ApiError> {
    ...
    if exists {
        ...
        Err(ApiError::BadRequest("social post not found".to_string()))
    }
}
...
async fn ensure_visible_comment_exists(state: &AppState, comment_id: Uuid) -> Result<(), ApiError> {
    ...
    if exists {
        ...
        Err(ApiError::BadRequest("social comment not found".to_string()))
    }
}

async fn ensure_comment_belongs_to_post(
    state: &AppState,
    post_id: Uuid,
    comment_id: Uuid,
) -> Result<(), ApiError> {
...
fn social_profile_response_from_row(config: &crate::config::AppConfig, row: SocialProfileRow) -> SocialProfileResponse {
    let avatar_optimized_urls = row
        .avatar_url
        .as_deref()
        .and_then(|url| optimized_public_image_urls(config, url, "image/jpeg"));
...
fn social_media_attachment_from_row(config: &crate::config::AppConfig, row: SocialMediaAttachmentRow) -> SocialMediaAttachment {
    let optimized_urls = row
        .public_url
        .as_deref()
        .and_then(|url| optimized_public_image_urls(config, url, &row.mime_type));
...
fn clean_optional_details(input: Option<&str>) -> Result<String, ApiError> {
    input
        .map(|value| clean_body(value, "details", 2000))
        .transpose()
...
fn normalize_post_kind(input: &str) -> Result<String, ApiError> {
    ...
    match value.as_str() {
        ...
        _ => Err(ApiError::BadRequest(
            "kind must be post, reflection, question, proof, milestone, or update".to_string(),
        )),
    }
}
...
fn normalize_post_visibility(input: &str) -> Result<String, ApiError> {
    ...
    match value.as_str() {
        ...
        _ => Err(ApiError::BadRequest(
            "visibility must be public or community for the public social feed".to_string(),
        )),
    }
}
...
fn normalize_reaction_kind(input: &str) -> Result<String, ApiError> {
    ...
    match value.as_str() {
        ...
        _ => Err(ApiError::BadRequest(
            "reaction kind must be like, support, insightful, or celebrate".to_string(),
        )),
    }
}
...
fn normalize_report_reason(input: &str) -> Result<String, ApiError> {
    let value = input.trim().to_ascii_lowercase();
    match value.as_str() {
        "spam" | "abuse" | "harassment" | "unsafe" | "privacy" | "misleading" | "other" => {
            Ok(value)
        }
        _ => Err(ApiError::BadRequest(
            "reason must be spam, abuse, harassment, unsafe, privacy, misleading, or other"
                .to_string(),
        )),
    }
}
